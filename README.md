...
Trained on two corpora to validate the architecture's generality and we name the submodels as:
    - ShakespeareGPT (Trained on TinyShakespeare)
    - WeekndGPT (Trained on 300+ Weeknd Songs fetched via LyricsGenius)

## Architecture Overview:

- Tokenizer: The tokenizer's purpose is to break down our sentences into tokens. 1 token is roughly 3/4th of a word.

- Embeddings: These are the numerical representations of the tokens in multi-dimensional space.

- Positional Encodings: The positional information of the tokens. Helps the model understand that "dog bites man" is distinct from "man bites dog" or "man dog bites".

- Masked Multi-Head Attention: We linearly project the queries, keys and values h times with different, learned linear projections to dq, dk, and dv. We perform the attention function in parallel on each of these projected versions.

- FeedForward: In the transformer architecture specifically, two linear projections with a ReLU in between. Input (d_model) -> Linear -> ReLU -> Linear -> Output(d_model)
...
```
TokenEmbedding -> PositionalEncoding -> N × DecoderBlock -> LM Head
...
Word-level tokenization is often superior to the character-level, but it requires significantly more data to work. With only 144K tokens across a 5421 word vocab, most words appeared too infrequently for the model to capture the meaningful context. Character-level takes the win for a small corpus by reducing the vocabulary to 167 characters, giving the model 819K training examples to learn from.
...
**Context Degradation:** The Generation quality for the character-level model degrades after ~150 tokens(characters). Corpus capacity is the bottleneck limiting us to a miniature model — not a bug.

## What's Next? — R&B GPT

- Scrape Drake, SZA, Partynextdoor, Nav, Chris Brown
- Combine these artists with The Weeknd Corpus

This will allow the word-level tokenizer to use its full potential.
